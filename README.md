...
- роль модератора позволяет управлять обычными пользователями без доступа к настройкам, маршрутам и конфигурации серверов;
...
Администратор и модератор открывают отдельный раздел `Пользователи` в главном меню:

- **Администратор** управляет всеми учетными записями, назначает роли, настраивает маршруты, протоколы и резервные копии серверов;
- **Модератор** создает и изменяет только обычных пользователей, но не может управлять администраторами, другими модераторами или собственной учетной записью;
- **Пользователь** не видит раздел управления пользователями и работает только с разрешенными серверами и собственными подключениями;
- настройки панели и маршруты доступны только администратору;
- установка и удаление протоколов, импорт конфигурации, создание и восстановление резервных копий серверов доступны только администратору;

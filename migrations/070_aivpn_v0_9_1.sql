-- =====================================================================
-- Migration 070: Update AIVPN to release v0.9.1
-- Pin the server binary to the tagged GitHub release asset instead of the
-- moving master branch, and check out the matching repo tag for mask-assets.
-- Release: https://github.com/infosave2007/aivpn/releases/tag/v0.9.1
-- =====================================================================
...
# Allow forwarding for the VPN subnet. On Docker hosts the FORWARD policy is DROP,
# and the AIVPN TUN interface is not covered by Docker''s own FORWARD rules, so without
# these explicit ACCEPT rules client traffic is forwarded out (MASQUERADE) but the
# return path is dropped (clients connect/handshake but get no traffic — bytes_out=0).
iptables -C FORWARD -s 10.0.0.0/24 -j ACCEPT 2>/dev/null || iptables -I FORWARD 1 -s 10.0.0.0/24 -j ACCEPT
iptables -C FORWARD -d 10.0.0.0/24 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT 2>/dev/null || iptables -I FORWARD 1 -d 10.0.0.0/24 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT

# Get external IP
EXTERNAL_IP=$(curl -s -4 ifconfig.me 2>/dev/null || curl -s -4 icanhazip.com 2>/dev/null || echo "YOUR_SERVER_IP")

# Clone AIVPN repository at the target release tag (mask-assets must match the binary)
AIVPN_DIR="/opt/amnezia/aivpn"
if [ ! -d "$AIVPN_DIR/.git" ]; then
  echo "Cloning AIVPN repository at $AIVPN_VERSION..."
  rm -rf "$AIVPN_DIR"
  git clone --depth=1 --branch "$AIVPN_VERSION" https://github.com/infosave2007/aivpn.git "$AIVPN_DIR"
else
  echo "Updating AIVPN repository to $AIVPN_VERSION..."
  git -C "$AIVPN_DIR" fetch --depth=1 origin "tag" "$AIVPN_VERSION"
  git -C "$AIVPN_DIR" checkout -f "$AIVPN_VERSION"
fi
...
DOWNLOAD_URL="https://github.com/infosave2007/aivpn/releases/download/${AIVPN_VERSION}/aivpn-server-linux-x86_64"
...
# Copy mask assets to preset directory (v0.9.1 moved them under aivpn-common/)
